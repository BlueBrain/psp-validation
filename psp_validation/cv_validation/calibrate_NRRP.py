...
JK CV).
See Barros-Zulaica et al 2019; last update: András Ecker, 06.2021
"""
...
def _sample_cvs(unique_nrrps, counts, all_cvs):
    ...
    for nrrp, count in zip(unique_nrrps, counts):
        cvs.extend(np.random.choice(all_cvs[f"nrrp{nrrp}"]["CV"],
                                    size=count, replace=False))
        jk_cvs.extend(np.random.choice(all_cvs[f"nrrp{nrrp}"]["JK_CV"],
                                       size=count, replace=False))
...
def regress_lambdas(lambdas, cvs, target_cv, jk_cvs, target_jk_cv):
    ...
    if np.max(cvs) <= target_cv:
        L.info("All CVs are lower than %.2f, consider it univesicular (NRRP = 1) and check U_SE",
               target_cv)
    else:
    ...
    if np.max(jk_cvs) <= target_jk_cv:
        L.info("All JK CVs are lower than %.2f, consider it univesicular (NRRP = 1) and check U_SE",
               target_jk_cv)
    else:
        best_jk_lambda = lambdas[np.argmin(np.abs(jk_cvs - target_jk_cv))]
        L.info("Calculated target JK CV = %.2f is best matched with NRRP = %.2f",
               target_jk_cv, best_jk_lambda)
...
def run_calibration(output_dir, pathways, pathway_name, nrrp, n_pairs=None, n_reps=None):
    ...
    if n_pairs is None:
        ...
        raise ValueError((f"number of pairs to choose (given: {n_pairs} must be less than number "
